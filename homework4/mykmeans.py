...
def plotClusters(option, clusters,centroids):
    ...
    if option==1:
        ...
        labelx="reviewCount"
        labely="checkins"
    colors = iter(pltcm.rainbow(np.linspace(0,1,len(clusters))))
    patches=[]
    for i,cluster in enumerate(clusters):
        x=[]
        y=[]
        for point in cluster:
            x.append(point[attribute1])
            y.append(point[attribute2])
        col = next(colors)
        plt.scatter(x,y,color=col)
        patch = mpatches.Patch(color=col, label=("Cluster"+str(i+1)))
        patches.append(patch)
        x=[]
        y=[]
    for centroid in centroids:
        x.append(centroid[attribute1])
        y.append(centroid[attribute2])
    patches.append(mpatches.Patch(color='black',label="Centroids"))
    ...
    while True:
        ...
        if same == False:
            ...
            if clusteringOption != 5:
                if plotOption == "1":
                    plotClusters(1,clusters,centroids)
                elif plotOption == "2":
                    plotClusters(2,clusters,centroids)
